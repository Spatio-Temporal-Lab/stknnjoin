...
def st_knn_join(x):
    ...
    for i in range(cnt):
        ...
        if os.system(f'hdfs dfs -test -e {file_save_path}/part-00000') != 0:
            executor.execute(alpha=args.knobs_setting["binNum"], beta=args.knobs_setting["binNum"],
...
def search_online():
    ...
    data_model = Data_model(args=args)
    model = Model()
    ...
    if args.data_type == 'point':
        nums_r, timerange_r, longitude_range_r, latitude_range_r, sp_dist_r = data_r.data_info()
        nums_s, timerange_s, longitude_range_s, latitude_range_s, sp_dist_s = data_s.data_info()
        ...
        nums_r, timerange_r, longitude_range_r, latitude_range_r, sp_dist_r, avg_p_r, dist_r = data_r.data_info_line()
        nums_s, timerange_s, longitude_range_s, latitude_range_s, sp_dist_s, avg_p_s, dist_s = data_s.data_info_line()
        ...
        nums_r, timerange_r, longitude_range_r, latitude_range_r, sp_dist_r, avg_p_r, area_r = data_r.data_info_polygon()
        nums_s, timerange_s, longitude_range_s, latitude_range_s, sp_dist_s, avg_p_s, area_s = data_s.data_info_polygon() 
    ...
    res = gp(st_knn_join_online, 50, 30)
    # 记录搜索结束时间
    end_time = time.time()
...
# gp回归
def gp(function=st_knn_join, n_calls=100, n_random_starts=60):
    # 设置三个参数范围
    # todo 范围能以参数形式传入设置
    if args.knob == 'alpha':
        bound = [(50, 1000)]
...
def record(execute_time):
    data_r = Data(args.data_file_r_local)
    data_s = Data(args.data_file_s_local)
    nums_r, timerange_r, longitude_range_r, latitude_range_r, sp_dist_r = data_r.data_info()
    nums_s, timerange_s, longitude_range_s, latitude_range_s, sp_dist_s = data_s.data_info()
    k = args.k
    with open(f'{args.output_dir}/record', 'a') as f:
        # 写入特征
...
def record_line(execute_time):
    data_r = Data(args.data_file_r_local)
    data_s = Data(args.data_file_s_local)
    nums_r, timerange_r, longitude_range_r, latitude_range_r, sp_dist_r, avg_point_r, distance_r = data_r.data_info_line()
    nums_s, timerange_s, longitude_range_s, latitude_range_s, sp_dist_s, avg_point_s, distance_s = data_s.data_info_line()
    k = args.k
    with open(f'{args.output_dir}/record', 'a') as f:
        # 写入特征
...
def record_polygon(execute_time):
    data_r = Data(args.data_file_r_local)
    data_s = Data(args.data_file_s_local)
    nums_r, timerange_r, longitude_range_r, latitude_range_r, sp_dist_r, avg_point_r, area = data_r.data_info_polygon()
    nums_s, timerange_s, longitude_range_s, latitude_range_s, sp_dist_s, avg_point_s, area = data_s.data_info_polygon()
    k = args.k
    with open(f'{args.output_dir}/record', 'a') as f:
        # 写入特征
        f.write(f'{nums_r}\t{timerange_r}\t{longitude_range_r}\t{latitude_range_r}\t{avg_point_r}\t{area}\t')
        for sp_d_r in sp_dist_r:
            for x in sp_d_r:
                f.write(f'{int(x)}\t')
        f.write(f'{nums_s}\t{timerange_s}\t{longitude_range_s}\t{latitude_range_s}\t{avg_point_s}\t{area}\t')
        for sp_d_s in sp_dist_s:
            for y in sp_d_s:
                f.write(f'{int(y)}\t')
        f.write(f'{k}\t')
